...
- Track how long each session and the full build take
- Stop early if `BUILD_PROGRESS.md` indicates all phases are complete
- Post progress to Discord if `CLAUDE_DISCORD_WEBHOOK_URL` is set (see [Discord Notifications](#discord-notifications))
...
| `notify.sh` | Optional Discord notifier. No-ops unless `CLAUDE_DISCORD_WEBHOOK_URL` is set. |
...
Get build updates on your phone while the build runs overnight. Notifications are opt-in and the build behaves identically without them.
